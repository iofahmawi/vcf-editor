...
<style>
  ...
  *{box-sizing:border-box; -webkit-tap-highlight-color: transparent; outline:none;}
  ...
  label{ display:block; font-size:12.5px; font-weight:700; color:var(--muted); margin-bottom:11px; }
  ...
  .multi-row input[type=tel]{
    ...
    box-sizing:border-box; padding:11px 8px; line-height:1.6;
  }
  ...
  .multi-row.row-address select, .multi-row.row-address input.type-text{
    flex:0 0 102px; width:102px; padding:11px 6px;
  }
  ...
  #unifyGroupPickerList, #unifyGroupList{ scrollbar-width:thin; scrollbar-color: var(--border) transparent; }
  #unifyGroupPickerList::-webkit-scrollbar, #unifyGroupList::-webkit-scrollbar{ width:4px; }
  #unifyGroupPickerList::-webkit-scrollbar-track, #unifyGroupList::-webkit-scrollbar-track{ background:transparent; margin:6px 0; }
  #unifyGroupPickerList::-webkit-scrollbar-thumb, #unifyGroupList::-webkit-scrollbar-thumb{ background:var(--border); border-radius:10px; }
  ...
  .dup-merge-btn{
    color:var(--accent); background:var(--card2); border:none; border-radius:8px; padding:4px 10px 8px;
...
<header id="mainHeader" style="display:none;">
  ...
  <div style="position:relative; display:inline-block;">
    ...
    <div id="listMenuDropdown" style="display:none; position:absolute; top:calc(100% + 6px); left:0; background:var(--card); border:1px solid var(--border); border-radius:8px; box-shadow:0 4px 12px rgba(0,0,0,.1); min-width:147px; z-index:30; overflow:hidden;">
      <button type="button" class="link-btn" id="menuUndoOpt" disabled style="display:block; width:100%; text-align:right; padding:16px 14px; color:var(--text); font-size:13px; border-bottom:1px solid var(--border); cursor:default;"><span id="menuUndoOptText" style="opacity:0.4;">تراجع</span></button>
...
<div id="multiPrefixModalOverlay" style="display:none; position:fixed; inset:0; background:rgba(20,24,38,0.45); backdrop-filter:blur(2px); z-index:100; align-items:flex-start; justify-content:center; padding:47px 20px 20px;">
  <div class="panel" style="width:100%; max-width:380px; margin:0; box-shadow:0 12px 32px rgba(0,0,0,.18); max-height:calc(100vh - 94px); display:flex; flex-direction:column;">
    <div style="font-family:var(--font-display); font-weight:800; font-size:15px; margin-bottom:6px; color:var(--text); text-align:center;">تعديل البادئة</div>
    ...
    <div id="multiPrefixList" style="overflow-y:auto; flex:1; margin-bottom:14px;"></div>
    <div style="display:flex; gap:8px;">
...
<div id="saveFormatModalOverlay" style="display:none; position:fixed; inset:0; background:rgba(20,24,38,0.45); backdrop-filter:blur(2px); z-index:100; align-items:center; justify-content:center; padding:20px;">
  <div class="panel" style="width:100%; max-width:320px; margin:0; box-shadow:0 12px 32px rgba(0,0,0,.18);">
    ...
    <div style="font-size:12px; color:var(--muted); margin-top:20px; line-height:normal; text-align:center;">الصور تصدر فقط مع ملفات VCF</div>
  </div>
</div>
...
<div id="mergeModalOverlay" style="display:none; position:fixed; inset:0; background:rgba(20,24,38,0.45); backdrop-filter:blur(2px); z-index:100; align-items:center; justify-content:center; padding:20px;">
  <div class="panel" style="width:100%; max-width:340px; margin:0; box-shadow:0 12px 32px rgba(0,0,0,.18); max-height:calc(100vh - 40px); overflow-y:auto;">
...
<div id="bulkPhotoModalOverlay" style="display:none; position:fixed; inset:0; background:rgba(20,24,38,0.45); backdrop-filter:blur(2px); z-index:100; align-items:flex-start; justify-content:center; padding:47px 20px 20px;">
  <div class="panel" style="width:100%; max-width:340px; margin:0; box-shadow:0 12px 32px rgba(0,0,0,.18); max-height:86vh; overflow-y:auto; display:flex; flex-direction:column;">
    ...
    <div id="bulkPhotoListWrap" style="display:none; border:1.07px solid var(--border-light); border-radius:10px; margin-bottom:14px; overflow:hidden;">
      <div id="bulkPhotoList" style="max-height:34vh; overflow-y:auto;"></div>
    </div>
...
<div id="embedPhotosModalOverlay" style="display:none; position:fixed; inset:0; background:rgba(20,24,38,0.45); backdrop-filter:blur(2px); z-index:100; align-items:flex-start; justify-content:center; padding:47px 20px 20px;">
  <div class="panel" style="width:100%; max-width:340px; margin:0; box-shadow:0 12px 32px rgba(0,0,0,.18); max-height:86vh; overflow-y:auto; display:flex; flex-direction:column;">
    ...
    <div id="embedPhotosListWrap" style="display:none; border:1.07px solid var(--border-light); border-radius:10px; margin-bottom:18px; overflow:hidden;">
      <div id="embedPhotosList" style="max-height:40vh; overflow-y:auto;"></div>
    </div>
...
<div id="unifyGroupModalOverlay" style="display:none; position:fixed; inset:0; background:rgba(20,24,38,0.45); backdrop-filter:blur(2px); z-index:100; align-items:flex-start; justify-content:center; padding:25px 20px 20px;">
  <div class="panel" style="width:100%; max-width:340px; margin:0; box-shadow:0 12px 32px rgba(0,0,0,.18); max-height:94vh; overflow-y:auto; display:flex; flex-direction:column;">
    ...
    <div class="field-group" style="margin-bottom:20px;">
      ...
      <div style="border:1.07px solid var(--border-light); border-radius:10px; overflow:hidden; position:relative;">
        <div id="unifyGroupPickerList" style="max-height:131px; overflow-y:auto; overscroll-behavior:contain; -webkit-overflow-scrolling:touch; touch-action:pan-y; scrollbar-width:none;"></div>
    ...
    <div id="unifyGroupListWrap" style="display:none; border:1.07px solid var(--border-light); border-radius:10px; margin-bottom:14px; overflow:hidden; flex-shrink:0;">
      <div id="unifyGroupList" style="height:167px; min-height:167px; overflow-y:auto; overscroll-behavior:contain; -webkit-overflow-scrolling:touch; touch-action:pan-y; flex-shrink:0;"></div>
    </div>
...
<div id="nameFormatModalOverlay" style="display:none; position:fixed; inset:0; background:rgba(20,24,38,0.45); backdrop-filter:blur(2px); z-index:100; align-items:flex-start; justify-content:center; padding:47px 20px 20px;">
  <div class="panel" style="width:100%; max-width:340px; margin:0; box-shadow:0 12px 32px rgba(0,0,0,.18); max-height:90vh; overflow-y:auto; display:flex; flex-direction:column;">
    ...
    <div style="display:flex; align-items:center; justify-content:flex-start; margin-bottom:6px; padding:0 4px;">
      <button type="button" id="nameFormatShowAllBtn" class="link-btn" style="color:var(--muted); font-size:12.5px; font-weight:700; background:none; padding:4px;">عرض الكل</button>
    </div>

    <input type="text" id="nameFormatSearch" placeholder="ابحث عن جهات الاتصال" style="width:100%; box-sizing:border-box; padding:8px 14px; line-height:2; border-radius:10px; border:1.07px solid var(--border-light); background:transparent; color:var(--text); font-size:13.5px; margin-bottom:10px;">
    ...
    <div id="nameFormatListWrap" style="display:none; border:1.07px solid var(--border-light); border-radius:10px; margin-bottom:14px; overflow:hidden;">
      <div id="nameFormatList" style="max-height:223px; overflow-y:auto;"></div>
    </div>

    <div class="field-group" style="margin-bottom:14px;">
      <label>الصيغة الجديدة</label>
      <input type="text" id="nameFormatValue" placeholder="الكلمة التي ستحل محل الكلمة المطابقة في الاسم">
    </div>
...
function saveUndoSnapshot(description){
  ...
  if(btn){
    ...
    const txt = $('#menuUndoOptText');
    if(txt) txt.style.opacity = '1';
  }
}

// تصفّر لقطة التراجع وتعطّل زر "تراجع" — تُستدعى عند بدء ملف جديد كلياً
  ...
  for(const line of lines){
    ...
    if(upper.startsWith('BEGIN:VCARD')){
      cur = { id: 'c'+Math.random().toString(36).slice(2,9), fn:'', nickname:'', org:'', title:'', note:'', adrs:[], categories:'', bday:'',
              phones:[], emails:[], urls:[], rawExtra:[], version:'3.0', photo:null, _itemRefs:{} };
      continue;
    }
...
function renderList(filter=''){
  ...
  const filtered = getFilteredContacts(filter).slice().sort((a,b) => {
    ...
    return sortReversed ? b.fn.localeCompare(a.fn, 'ar') : a.fn.localeCompare(b.fn, 'ar');
  });
...
$('#menuEmbedLinkedPhotosOpt').addEventListener('click', () => {
  ...
  if(pendingCount === 0){ toast('لا توجد صور مرتبطة بحاجة للتضمين'); return; }
  closeListMenuDropdown();
...
function renderBulkPhotoList(){
  ...
  const matched = bulkPhotoMatchContacts(q);
  selectAllBtn.style.display = matched.length ? 'inline-block' : 'none';
...
$('#bulkPhotoSearch').addEventListener('input', renderBulkPhotoList);
...
$('#bulkPhotoConfirm').addEventListener('click', () => {
  ...
  let count = 0;
  contacts.forEach(c => {
    if(bulkPhotoSelectedIds.has(c.id)){ c.photo = bulkPhotoDataUrl || null; count++; }
  });
  renderList($('#search').value);
...
function resetUnifyGroupFields(){
...
function renderUnifyGroupList(){
  ...
  const matched = isGroupMode ? groupMembers : unifyGroupMatchContacts(q);
  selectAllBtn.style.display = matched.length ? 'inline-block' : 'none';
...
$('#unifyGroupSelectAll').addEventListener('click', () => {
  const q = unifyGroupPickedName !== null ? unifyGroupPickedName : $('#unifyGroupSearch').value.trim();
  const matched = unifyGroupMatchContacts(q);
  const allSelected = matched.length > 0 && matched.every(c => unifyGroupSelectedIds.has(c.id));
...
function updateNameFormatConfirmState(){
  ...
  const ready = nameFormatSelectedIds.size > 0 && (!!selectedCaseStyle || $('#nameFormatValue').value.trim().length > 0);
  btn.style.opacity = ready ? '1' : '.5';
  btn.style.pointerEvents = ready ? 'auto' : 'none';
}
...
$('#nameFormatConfirm').addEventListener('click', () => {
  ...
  if(!val || !query) return;
  saveUndoSnapshot('تعديل صيغة اسم ' + nameFormatSelectedIds.size + ' جهة اتصال');
  let count = 0;
  contacts.forEach(c => {
...
function buildMergePlan(ids, choices){
  ...
  return {
    ...
    merged: { id: list[0].id, fn, nickname, org, dept, title, note, adrs, categories, bday, phones, emails, urls, rawExtra, version: list[0].version || '3.0', photo, photoUrl },
...
function openMergeModal(plan, opts){
  ...
  if(plan.conflicts.name){
    ...
    customNameInput.style.cssText = 'width:100%; box-sizing:border-box; padding:13px 12px; background:transparent; color:var(--text); font-size:14px; font-weight:600; font-family:var(--font-body); line-height:normal;';
  ...
  textConflictFields.forEach(({ key, label: labelText, placeholder }) => {
    ...
    customInput.className = 'merge-custom-name';
    customInput.style.cssText = 'width:100%; box-sizing:border-box; padding:10px 12px; background:transparent; color:var(--text); font-size:13.5px; font-weight:600; font-family:var(--font-body); line-height:normal;';
    customInput.addEventListener('input', () => {
      group.querySelectorAll('.merge-opt').forEach(o => o.classList.remove('selected'));
      const v = customInput.value.trim();
      if(v){
